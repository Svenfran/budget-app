...
public interface CartRepository extends CrudRepository<Cart, Long> {
    ...
    @Query("""
            select new com.github.svenfran.budgetapp.budgetappbackend.dto.SpendingsOverviewMonthlyTotalSumAmountDto(year(c.datePurchased) as _year, month(c.datePurchased) as _month, sum(c.amount))
            from Cart as c where year(c.datePurchased) = :year and c.group.id = :groupId and c.isDeleted = false
    ...
    @Query("""
            select new com.github.svenfran.budgetapp.budgetappbackend.dto.SpendingsOverviewYearlyTotalSumAmountDto(year(c.datePurchased) as _year, sum(c.amount))
            from Cart as c where c.group.id = :groupId and c.isDeleted = false
            group by _year
            order by _year desc
            """)
    ...
    @Query("""
            select sum(c.amount)
            from Cart as c
            where c.group.id = :groupId and c.isDeleted = false
           """)
    ...
    @Query("""
            select new com.github.svenfran.budgetapp.budgetappbackend.dto.SpendingsOverviewAverageDto(
            gmh.userId, sum(c.averagePerMember), month(c.datePurchased) as _month, year(c.datePurchased) as _year)
    ...
    @Query("""
            select new com.github.svenfran.budgetapp.budgetappbackend.dto.SpendingsOverviewAverageDto(
            gmh.userId, sum(c.averagePerMember), year(c.datePurchased) as _year)
            from GroupMembershipHistory as gmh join Cart as c on gmh.groupId = c.group.id
    ...
    @Query("""
            select new com.github.svenfran.budgetapp.budgetappbackend.dto.SpendingsOverviewAmountAverageDiffPerMonthDto(
            sum(case when c.user.id = gmh.userId then c.amount else 0 end), 0.0, 0.0, gmh.userId, year(c.datePurchased) as _year, month(c.datePurchased) as _month)
            from GroupMembershipHistory as gmh join Cart as c on gmh.groupId = c.group.id
    ...
    @Query("""
            select new com.github.svenfran.budgetapp.budgetappbackend.dto.SpendingsOverviewAmountAverageDiffPerUserDto(
            sum(case when c.user.id = gmh.userId then c.amount else 0 end), 0.0, 0.0, gmh.userId, year(c.datePurchased) as _year)
            from GroupMembershipHistory as gmh join Cart as c on gmh.groupId = c.group.id
            where year(c.datePurchased) = :year and gmh.groupId = :groupId and c.isDeleted = false 
            and cast(gmh.membershipStart as date) <= c.datePurchased
            and ((cast(gmh.membershipEnd as date) > c.datePurchased) or gmh.membershipEnd IS NULL)
    ...
    @Query("""
            select new com.github.svenfran.budgetapp.budgetappbackend.dto.SpendingsOverviewAmountAverageDiffPerUserDto(
            sum(case when c.user.id = gmh.userId then c.amount else 0 end), 0.0, 0.0, gmh.userId) 
            from GroupMembershipHistory as gmh join Cart as c on gmh.groupId = c.group.id
            where gmh.groupId = :groupId and c.isDeleted = false 
            and cast(gmh.membershipStart as date) <= c.datePurchased
            and ((cast(gmh.membershipEnd as date) > c.datePurchased) or gmh.membershipEnd IS NULL)
